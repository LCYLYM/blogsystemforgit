...
class GMEEK():
    ...
    def createPlistHtml(self):
        ...
        keys=list(OrderedDict.fromkeys(
            ...
            + list(self.blogBase.get("exlink", {}).keys())
    ...
    def createFeedXml(self):
        ...
        feedFile=open(self.root_dir+'new.xml','w',encoding='utf-8')
        feedFile.write(feed.rss_str(pretty=True).decode('utf-8'))
        feedFile.close()
...
blog.blogBase["postListJson"]=dict(sorted(blog.blogBase["postListJson"].items(),key=lambda x:x[1]["createdAt"],reverse=True))
